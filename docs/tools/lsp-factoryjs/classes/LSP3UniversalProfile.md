...
Returns an rxjs Observable which emits events as UP contracts are deployed
...
```javascript
lspFactory.LSP3UniversalProfile.getDeployedByteCode(
...
Uploads the LSP3Profile data to the desired endpoint. This can be an `https` URL either pointing to
...
Returns processed LSP3 Data and upload url
