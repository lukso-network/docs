...
There is a growing need for more operations beyond the basic **CALL** and **CREATE** functionality of an externally owned account (EOA). These two operations enables interaction with other addresses on the blockchain to perform tasks such as calling functions, transferring native tokens, and creating new contracts. As the number of use cases for smart contracts increases, so too does the need for additional functionality.
