...
In the [**Read Profile Data** guide](../universal-profile/read-profile-data), we learned how to read the Universal Profile properties and use the data key names with the `fetchData()` function of the [erc725.js](../../tools/erc725js/getting-started/) library. In the same way, we can now fetch all the assets owned by the Universal Profile by fetching the `LSP5ReceivedAssets[]` metadata.
...
Using same method we used to fetch received assets, we can fetch all assets ever created by the Universal Profile by fetching the `LSP12IssuedAssets[]` metadata using the [erc725.js](../../tools/erc725js/getting-started/) library.

```javascript title="read_assets.js"
// ...

// Fetch the ever issued assets from the Universal Profile
const profile = new ERC725(UniversalProfileSchema, address, provider, config);
const result = await profile.fetchData('LSP12IssuedAssets[]');
const issuedAssets = result.value;
...
const asset = new web3.eth.Contract(LSP4.abi, SAMPLE_ASSET_ADDRESS);


// Check if the contract supports the ERC725Y interface
const isERC725Y = await asset.methods.supportsInterface(ERC725Y_INTERFACE_IDS['3.0']).call();
